...
public class DadkvsClient {
	...
	private int findLeader() {
		int testKey = 0;
		DadkvsMain.ReadRequest.Builder readRequest = DadkvsMain.ReadRequest.newBuilder()
				.setReqid(-1)
				.setKey(testKey);

		ArrayList<DadkvsMain.ReadReply> readResponses = new ArrayList<>();
		GenericResponseCollector<DadkvsMain.ReadReply> readCollector = new GenericResponseCollector<>(readResponses,
				n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DadkvsMain.ReadReply> readObserver = new CollectorStreamObserver<>(readCollector);
			async_stubs[i].read(readRequest.build(), readObserver);
		}

		readCollector.waitForTarget(1);

		if (!readResponses.isEmpty()) {
			DadkvsMain.ReadReply firstReply = readResponses.get(0);
			for (int i = 0; i < n_servers; i++) {
				if (firstReply.getReqid() == -1) {
					return i;
				}
			}
		}
	...
	private boolean doCommit(int key1, int key1_version, int key2, int key2_version, int write_key, int write_value) {
		...
		GenericResponseCollector<DadkvsMain.CommitReply> commit_collector = new GenericResponseCollector<>(
				commit_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DadkvsMain.CommitReply> commit_observer = new CollectorStreamObserver<>(
					commit_collector);
			async_stubs[i].committx(commit_request.build(), commit_observer);
		}

		commit_collector.waitForTarget(1);

		if (!commit_responses.isEmpty()) {
			DadkvsMain.CommitReply commit_reply = commit_responses.get(0);
			System.out.println("Reqid = " + reqid + " id in reply = " + commit_reply.getReqid());
			result = commit_reply.getAck();
			if (result) {
				System.out.println("Committed key " + commit_request.getWritekey() + " with value "
						+ commit_request.getWriteval());
			} else {
				System.out.println("Commit Failed");
			}
		} else {
			System.out.println("Panic...error committing");
		}

		commit_collector.waitForTarget(n_servers);
	...
	private VersionedValue doRead(int key) {
		...
		GenericResponseCollector<DadkvsMain.ReadReply> read_collector = new GenericResponseCollector<>(read_responses,
				n_servers);
		...
		read_collector.waitForTarget(1);

		if (!read_responses.isEmpty()) {
			DadkvsMain.ReadReply read_reply = read_responses.get(0);
			...
			return new VersionedValue(read_reply.getValue(), read_reply.getTimestamp());
		} else {
	...
	private void doTransactions() throws Exception {
		...
		System.out.println("going to run " + loop_size + " transactions with key range = " + key_range
				+ " sleep delay range = " + sleep_range);
				...
				case "read":
					...
					if (parameter1 != null) {
						try {
							...
							if (kv_entry != null)
								System.out.println("did read " + key + " with value " + kv_entry.getValue()
										+ " and version " + kv_entry.getVersion());
							else
					...
					break;
				case "tx":
					System.out.println("tx reading key " + parameter1 + " and key " + parameter2 + " : writting key "
							+ parameter3);
						...
						try {
							...
							if (write_key == 0)
								...
								if ((kv_entry1 != null) && (kv_entry2 != null))
									doCommit(read_key1, kv_entry1.getVersion(), read_key2, kv_entry2.getVersion(),
	...
	public void main_loop(String[] args) throws Exception {
		...
		System.out.println(
				"Client key_range = " + key_range + " sleep_range = " + sleep_range + " loop_size = " + loop_size);
